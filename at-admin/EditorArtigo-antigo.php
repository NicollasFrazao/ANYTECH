	...
	<body>
		...
		<div id="editor" class="menu-info">
			<form id="Frm_Artigo" method="POST" action="php/<?php if ($artigoEncontrado == 1) {echo 'AlterarArtigo';} else {echo 'PublicarArtigo';} ?>.php" enctype="multipart/form-data">
				...
				<table style="display: inline-block;">
					...
					<tr>
						<td>
							<input type="checkbox" id="chk_autor_oculto" name="oculto" value="1" <?php if ($artigoEncontrado == 1) {echo (($autorOculto == 1) ? 'checked' : '');} ?>/><label onclick="chk_autor_oculto.click();">Autor oculto</label>
						</td>
					</tr>
				</table>
			...
			<div class="button-box">
				<table width="100%" cellspacing="0">
					...
					<tr>
						<td>
							<input type="button" class="at-button" value="<?php if ($artigoEncontrado == 1) {echo 'Finalizar Alterações';} else {echo 'Publicar Artigo';} ?>" style="width: auto;" onclick="btn_publicar.click();"/>
						</td>
					</tr>
				</table>
			</div>
		...
		<div class="selectTags" id="artImages">
			...
			<div class="buttonstag" align="right">
				...
				<input type="button" value="Salvar" class="btntag cancel" onclick="txt_banner.value = ((bannerAdicionar != '') ? 'artigo/' + bannerAdicionar : ''); txt_thumbnail.value = ((thumbAdicionar != '') ? 'artigo/' + thumbAdicionar : ''); addPhotosArtClose();">
			</div>
		</div>
			...
			if (aux == 1)
			{
				...
				if ((file = capafile.files[0])) 
				{
					...
					img.onload = function () 
					{
						...
						{
							LerImagem(capafile, btn_capa);
				
							AjaxForm(Frm_Banner, "", "var retorno = this.responseXML; var mensagem = retorno.getElementById('lbl_aviso').textContent.trim(); adicionou = retorno.getElementById('ic_adicionou').textContent.trim(); nome = retorno.getElementById('nm_arquivo').textContent.trim(); if (adicionou == 1) {bannerAdicionar = nome;} else {alert(mensagem);}");
						}
						else
						{
			...
			if (aux == 1)
			{
				...
				if ((file = thumbfile.files[0])) 
				{
					...
					img.onload = function () 
					{
						...
						{
							LerImagem(thumbfile, btn_thumb);
				
							AjaxForm(Frm_Thumb, "", "var retorno = this.responseXML; var mensagem = retorno.getElementById('lbl_aviso').textContent.trim(); adicionou = retorno.getElementById('ic_adicionou').textContent.trim(); nome = retorno.getElementById('nm_arquivo').textContent.trim(); if (adicionou == 1) {thumbAdicionar = nome;} else {alert(mensagem);}");
						}
						else
						{
							alert('Por favor, inserir uma imagem com as dimensões de 350x350 pixels. As dimensões dessa imagem são de ' + this.width + 'x' + this.height + ' pixels.');
						}
					};
