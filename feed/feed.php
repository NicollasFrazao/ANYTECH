						...
						?>	
										<div class="anytech-post-box">
											...
											<div class="anytech-post-buttons"  align="center">
												...
												<input type="image" id="col_three_anytech_post_comment<?php echo $linha_Artigos['cd_artigo'].$cont; ?>" class="anytech-post-comment" src="<?php echo VOLTAR; ?>images/comment.png" onclick="showCommentBox('<?php echo $linha_Artigos['cd_artigo']; ?>', '<?php if ($linha_Artigos['nm_titulo'] == '') {echo 'Informação indisponível';} else {echo $linha_Artigos['nm_titulo'];} ?>', 'por <?php if ($linha_Artigos['ic_autor_oculto'] == 1) {echo 'ANYTECH';} else if ($linha_Artigos['nm_usuario'] == '') {echo 'Autor desconhecido';} else {echo $linha_Artigos['nm_usuario'];} ?>', '<?php echo $_SERVER['HTTP_HOST'].'/artigo/'.$linha_Artigos['ds_url']; ?>');">
												...
												<script>
													col_three_anytech_post_rating<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.onclick = function()
													{
														EsconderValores();
														col_three_anytech_post_rating<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = true;
														col_three_anytech_post_time<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = true;
														col_three_anytech_post_comment<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = true;
														col_three_anytech_post_favorite<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = true;
														col_three_anytech_post_share<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = true;
														var codigo = <?php echo $linha_Artigos['cd_artigo'].$cont; ?>;
														
														Ajax("GET", "<?php echo VOLTAR; ?>php/ContarAvaliacaoArtigo.php", "codigoArtigo=" + codigo, "var retorno = this.responseText; spt = retorno.split('&'); retorno = parseFloat(spt[0]); retorno = retorno.toFixed(1); at_avalreader.innerHTML = retorno; showRatingBox(spt[1],spt[2], '<?php echo $linha_Artigos['nm_titulo']; ?>','por <?php if ($linha_Artigos['ic_autor_oculto'] == 1) {echo 'ANYTECH';} else if ($linha_Artigos['nm_usuario'] == '')  {echo 'Autor desconhecido';} else {echo $linha_Artigos['nm_usuario'];} ?>','<?php echo VOLTAR."artigo/".$linha_Artigos['ds_url']; ?>','<?php echo $linha_Artigos['cd_artigo']; ?>'); col_three_anytech_post_rating<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = false; col_three_anytech_post_time<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = false;col_three_anytech_post_comment<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = false;col_three_anytech_post_favorite<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = false;col_three_anytech_post_share<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.disabled = false;");
													...
													col_three_anytech_post_favorite<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.onmouseover = function()
													{
														...
														if (<?php echo $logado ?> == 1)
														{
															Ajax("GET", "<?php echo VOLTAR; ?>php/ContarFavoritosArtigo.php", "codigoArtigo=" + codigo, "var retorno = this.responseText; var aux = retorno.split(';-;')[0]; var qnt = retorno.split(';-;')[1]; var conteudo = ''; if (aux == 1) {if (qnt > 1) {if (qnt - 1 > 1) {conteudo = 'Você e outras ' + (qnt - 1) + ' pessoas favoritaram';} else {conteudo = 'Você e outra pessoa favoritaram';}} else {conteudo = 'Você favoritou';}} else {conteudo = qnt + ((qnt == 1) ? ' pessoa favoritou' : ' pessoas favoritaram');} col_three_value_btn_favorite<?php echo $linha_Artigos['cd_artigo']; ?>.textContent = conteudo;");
														}
														else
														{
															Ajax("GET", "<?php echo VOLTAR; ?>php/ContarFavoritosArtigoOff.php", "codigoArtigo=" + codigo, "var retorno = this.responseText; col_three_value_btn_favorite<?php echo $linha_Artigos['cd_artigo']; ?>.textContent = retorno + ' favoritada' + ((retorno == 1) ? '' : 's');");
														}
													}
													...
													col_three_anytech_post_time<?php echo $linha_Artigos['cd_artigo'].$cont; ?>.onmouseover = function()
													{
														...
														if (<?php echo $logado ?> == 1)
														{
															Ajax("GET", "<?php echo VOLTAR; ?>php/ContarLeiturasArtigo.php", "codigoArtigo=" + codigo, "var retorno = this.responseText; var aux = retorno.split(';-;')[0]; var qnt = retorno.split(';-;')[1]; var conteudo = ''; if (aux == 1) {if (qnt > 1) {if (qnt - 1 > 1) {conteudo = 'Você e outras ' + (qnt - 1) + ' pessoas marcaram';} else {conteudo = 'Você e outra pessoa marcaram';}} else {conteudo = 'Você marcou';}} else {conteudo = qnt + ((qnt == 1) ? ' pessoa marcou' : ' pessoas marcaram');} col_three_value_btn_time<?php echo $linha_Artigos['cd_artigo']; ?>.textContent = conteudo;");
														}
														else
														{
															Ajax("GET", "<?php echo VOLTAR; ?>php/ContarLeiturasArtigoOff.php", "codigoArtigo=" + codigo, "var retorno = this.responseText; col_three_value_btn_time<?php echo $linha_Artigos['cd_artigo']; ?>.textContent = retorno + ' marcaç' + ((retorno == 1) ? 'ão' : 'ões');");
														}
													}
